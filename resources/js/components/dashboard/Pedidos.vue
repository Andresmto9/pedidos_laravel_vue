            ...
            getInfoSistema(){
                axios({
                    ...
                    if((response.data.pedido).length > 0){
                        let dataPedi = '';
                        $.each((response.data.pedido), function(key, value){
                            dataPedi += `
                                                ...
                                                <div class="  p-6">
                                                    <div class="pb-3 mb-4 border-b border-stone-200 text-xs font-medium flex justify-between text-blue-900">
                                                        <span class="flex items-center gap-1">
                                                            ...
                                                            Precio total del pedido: ${(value.pedido_precio_total)}
                                                        </span>
                                                    </div>
                                                    <h3 class="mb-4 font-semibold  text-2xl"><a href="" class="transition-all text-blue-900 hover:text-blue-600">${value.productos.producto_nombre}</a></h3>
                                                    <div class="overflow-auto">
                                                        <p class="text-sky-800 text-sm mb-0">
                                                            Canrtidad: ${value.pedido_cantidad}
                                                        </p>
                                                        <p class="text-sky-800 text-sm mb-0">
                                                            Valor del producto: ${value.productos.producto_valor}
                                                        </p>
                                                        <p class="text-sky-800 text-sm mb-0">
                                                            Sub total: ${value.pedido_precio_subtotal}
                                                        </p>
                                                        <p class="text-sky-800 text-sm mb-0">
                                                            Precio IVA: ${value.pedido_iva}
                                                        </p>
                                                        <p class="text-sky-800 text-sm mb-0">
                                                            IVA aplicado: ${value.productos.producto_iva} %
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <ul class="mx-auto grid max-w-full w-full grid-cols-3 gap-x-5 px-8">
                ...
                <div class="absolute bg-white shadow-lg left-0 p-6 border mt-2 border-indigo-300 rounded-lg w-[97vw] mx-auto transition-all duration-500 ease-in-out translate-x-40 opacity-0 invisible peer-checked:opacity-100 peer-checked:visible peer-checked:translate-x-1 md:ml-5">
                    <div class="grid place-items-center grid-cols-12">
                        <div class="col-span-12 grid w-full">
                            ...
                            <div class="grid grid-cols-12 col-span-12" v-html="pedidos"></div>
                        </div>
                    </div>
            ...
            <li class="">
                ...
                <div class="absolute bg-white shadow-lg left-0 p-6 border mt-2 border-indigo-300 rounded-lg w-[97vw] mx-auto transition-all duration-500 ease-in-out translate-x-40 opacity-0 invisible peer-checked:opacity-100 peer-checked:visible peer-checked:translate-x-1 md:ml-5">
                    <div class="col-span-12 grid w-full">
                        <div class="mb-4 text-center col-end-12 col-span-2">
                            <button class="transition duration-500 bg-teal-500 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button" @click="showModalUsuario = true">
                                Crear usuario
                            </button>
                        </div>
                    ...
                    <div class="grid grid-cols-12">
                        <UsuarioModal :mostrarUsuarioModal.sync="showModalUsuario"></UsuarioModal>
                    </div>
                </div>
            </li>
        </ul>
